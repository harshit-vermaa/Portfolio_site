...
// ============================================================
// 3D Hero Canvas — Interactive node graph
// ============================================================
const NODE_DATA = [
  { label: "Python", icon: "🐍", color: "#7c3aed", category: "ML" },
  { label: "Product Analytics", icon: "📊", color: "#06b6d4", category: "Core" },
  { label: "LangChain", icon: "🔗", color: "#10b981", category: "AI" },
  { label: "Data Pipelines", icon: "⚙️", color: "#f59e0b", category: "Eng" },
  { label: "A/B Testing", icon: "🧪", color: "#ec4899", category: "PM" },
  { label: "SQL", icon: "🗄️", color: "#6366f1", category: "Data" },
  { label: "PyTorch", icon: "🔥", color: "#7c3aed", category: "ML" },
  { label: "KPIs", icon: "🎯", color: "#06b6d4", category: "PM" },
  { label: "Computer Vision", icon: "👁️", color: "#10b981", category: "AI" },
  { label: "Dashboards", icon: "📈", color: "#f59e0b", category: "Data" },
  { label: "NLP", icon: "💬", color: "#ec4899", category: "AI" },
  { label: "Docker", icon: "🐳", color: "#6366f1", category: "Eng" },
];
...
function HeroCanvas({ darkMode }) {
  ...
  useEffect(() => {
    ...
    // Init nodes in a rough orbit around center
    const cx = W / 2, cy = H / 2;
    ...
    const center = {
      x: cx, y: cy,
      label: "Harshit", color: "#7c3aed", radius: 34, isCenter: true,
    };

    // Connections: each node connects to center + 2 nearest
    function getConnections(nodes) {
      const conns = [];
      nodes.forEach((n, i) => {
        conns.push({ from: -1, to: i }); // -1 = center
        // nearest neighbor
        ...
        if (nearest !== -1) conns.push({ from: i, to: nearest });
      });
      return conns;
    }
    ...
    function draw() {
      ...
      const centerNode = { ...center, x: W / 2, y: H / 2 };

      // Drift nodes gently
      nodes.forEach((n) => {
        n.angle += n.angleSpeed;
        const cx2 = W / 2 + Math.sin(t * 0.2) * 20;
        const cy2 = H / 2 + Math.cos(t * 0.15) * 15;
        n.x = cx2 + Math.cos(n.angle) * n.baseR;
        n.y = cy2 + Math.sin(n.angle) * n.baseR;
        n.pulse += 0.04;
        ...
        n.hovered = dist < n.radius + 8;
        if (dist < 80 && dist > 0) {
          n.x += (dx / dist) * (80 - dist) * 0.06;
          n.y += (dy / dist) * (80 - dist) * 0.06;
        }
      });

      // Draw connections
      const conns = getConnections(nodes);
      conns.forEach(({ from, to }) => {
        const a = from === -1 ? centerNode : nodes[from];
        const b = nodes[to];
        ...
        grad.addColorStop(0, `${a.color || "#7c3aed"}${Math.round(alpha * 255).toString(16).padStart(2, "0")}`);
        ...
        // Animated packet travelling the line
        const pct = (t * 0.4 + from * 0.3) % 1;
      ...
      // Draw center node
      const cglow = ctx.createRadialGradient(centerNode.x, centerNode.y, 0, centerNode.x, centerNode.y, 50);
      cglow.addColorStop(0, "rgba(124,58,237,0.25)");
      ...
      ctx.beginPath();
      ctx.arc(centerNode.x, centerNode.y, 30, 0, Math.PI * 2);
      const cgrad = ctx.createRadialGradient(centerNode.x, centerNode.y, 0, centerNode.x, centerNode.y, 30);
      cgrad.addColorStop(0, "#7c3aed");
      cgrad.addColorStop(1, "#06b6d4");
      ctx.fillStyle = cgrad;
      ctx.fill();
      ...
      ctx.fillText("HV", centerNode.x, centerNode.y);

      // Draw skill nodes
      nodes.forEach((n) => {
        const r = n.radius + (n.hovered ? 5 : 0) + Math.sin(n.pulse) * 1.5;
        // Glow
        ...
        ng.addColorStop(0, n.color + "ee");
        ng.addColorStop(1, n.color + "88");
        ctx.fillStyle = ng;
        ctx.fill();
        if (n.hovered) {
          ctx.strokeStyle = "#fff";
  ...
  const handleClick = useCallback((e) => {
    ...
    nodesRef.current.forEach((n) => {
      if (Math.hypot(n.x - mx, n.y - my) < n.radius + 8) found = n;
    });
    if (found) {
    ...
    <div style={{ position: "absolute", inset: 0, zIndex: 0 }}>
      ...
      {tooltip && (
        <div style={{
          ...
          border: `1px solid ${tooltip.node.color}60`,
          borderRadius: 10,
          padding: "10px 16px",
          pointerEvents: "none",
          backdropFilter: "blur(16px)",
          boxShadow: `0 8px 32px ${tooltip.node.color}30`,
          animation: "tooltipIn 0.2s ease",
        }}>
          <div style={{ fontSize: 11, fontFamily: "'JetBrains Mono', monospace", color: tooltip.node.color, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 4 }}>{tooltip.node.category}</div>
          <div style={{ fontSize: 15, fontWeight: 700, color: "#f1f5f9" }}>{tooltip.node.icon} {tooltip.node.label}</div>
        </div>
      )}
    </div>
  );
    ...
    <section style={{
      ...
      <div style={{ position: "relative", zIndex: 2, pointerEvents: "auto" }}>
        ...
        <p style={{
          ...
          ↑ Click the nodes to explore my skills
        </p>
    ...
    <Section id="gallery" style={{ padding: "100px 24px", overflow: "hidden" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        ...
        <p style={{ textAlign: "center", marginTop: 20, fontSize: 12, color: darkMode ? "#475569" : "#94a3b8", fontFamily: "'JetBrains Mono', monospace" }}>
          Click any panel to expand · Replace images in GALLERY_IMAGES config
        </p>
      </div>
    </Section>
  );
  ...
  return (
    <>
      ...
      <Section id="contact" style={{ padding: "100px 24px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          {/* Big headline like the design */}
          <div style={{ marginBottom: 60 }}>
            <h2 style={{
              fontSize: "clamp(32px, 5vw, 56px)", fontWeight: 800, letterSpacing: "-0.03em",
              color: darkMode ? "#f1f5f9" : "#0f172a", margin: "0 0 16px",
            ...
            <p style={{ fontSize: 17, color: darkMode ? "#64748b" : "#94a3b8", maxWidth: 560, lineHeight: 1.6, margin: 0 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "3fr 2fr", gap: 40, alignItems: "start" }}>
            ...
            <div style={{
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
                <div>
                  <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: darkMode ? "#64748b" : "#94a3b8", fontFamily: "'JetBrains Mono', monospace", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.07em" }}>Your Name</label>
                ...
                <div>
                  <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: darkMode ? "#64748b" : "#94a3b8", fontFamily: "'JetBrains Mono', monospace", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.07em" }}>Email Address</label>
              ...
              <div style={{ marginBottom: 16 }}>
                <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: darkMode ? "#64748b" : "#94a3b8", fontFamily: "'JetBrains Mono', monospace", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.07em" }}>Subject</label>
              ...
              <div style={{ marginBottom: 24 }}>
                <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: darkMode ? "#64748b" : "#94a3b8", fontFamily: "'JetBrains Mono', monospace", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.07em" }}>Message</label>
                <textarea style={{ ...inputStyle, resize: "vertical", minHeight: 130 }} placeholder="Tell me about the opportunity or what you'd like to discuss..." value={form.message} onChange={e => setForm(f => ({ ...f, message: e.target.value }))}
                  onFocus={e => e.target.style.borderColor = "#7c3aed"}
                  onBlur={e => e.target.style.borderColor = darkMode ? "rgba(255,255,255,0.12)" : "rgba(0,0,0,0.1)"} />
              </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              <div style={{
                ...
                <h3 style={{ margin: "0 0 16px", fontSize: 14, fontWeight: 700, color: darkMode ? "#e2e8f0" : "#1e293b" }}>Find me online</h3>
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  {[
                    ...
                    <div key={text} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      ...
                      <span style={{ fontSize: 13, color: darkMode ? "#94a3b8" : "#475569" }}>{text}</span>
                    </div>
                  ))}
                </div>

                <div style={{ height: 1, background: darkMode ? "rgba(255,255,255,0.07)" : "rgba(0,0,0,0.07)", margin: "20px 0 16px" }} />

                <div style={{ fontSize: 11, fontWeight: 700, color: darkMode ? "#475569" : "#94a3b8", fontFamily: "'JetBrains Mono', monospace", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 12 }}>Social Links</div>
              ...
              <div style={{
                ...
                <div style={{ fontSize: 11, fontWeight: 700, color: darkMode ? "#475569" : "#94a3b8", fontFamily: "'JetBrains Mono', monospace", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 12 }}>Response Time</div>
                <div style={{ fontSize: 28, fontWeight: 800, color: "#10b981", fontFamily: "'JetBrains Mono', monospace", marginBottom: 8 }}>Usually &lt; 24h</div>
                <div style={{ fontSize: 13, color: darkMode ? "#64748b" : "#94a3b8", lineHeight: 1.5 }}>Open to full-time roles, contract work, and collaborations.</div>
              </div>
            </div>
          </div>
        </div>
