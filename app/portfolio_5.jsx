...
function HeroCanvas({ darkMode }) {
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      // Arc center: horizontally centred, sits below the viewport bottom edge
      const arcCx = W / 2;
      const arcCy = H + 40;           // centre of the full circle is below the screen
      const arcR  = Math.min(W * 0.62, 560);  // radius of the arc

      // How many degrees of the circle we show (just the top portion):
      // We spread cards from ~210° to ~330° (top arc, 120° spread)
      const arcStartDeg = 210;
      const arcEndDeg   = 330;
      const arcSpanDeg  = arcEndDeg - arcStartDeg;
...
function Hero({ darkMode }) {
  ...
  return (
    <section style={{
      ...
      {/* ── Arc canvas — sits behind everything ── */}
      <HeroCanvas darkMode={darkMode} />

      {/* ── Deep centre glow (no text interference) ── */}
      <div style={{
      ...
      {/* ── Hero text content — pushed to upper 55% of viewport ── */}
      <div style={{
        position: "relative", zIndex: 2,
        width: "100%", maxWidth: 820,
        marginBottom: "8vh",   // breathing room above the arc
      }}>
        ...
        {/* Typewriter role */}
        <div style={{
          fontSize: "clamp(17px, 2.8vw, 26px)", fontWeight: 500, marginBottom: 18,
        ...
        <p style={{
          ...
          color: darkMode ? "#94a3b8" : "#64748b",
          maxWidth: 560, margin: "0 auto 32px", lineHeight: 1.65,
      ...
      {/* Scroll indicator — sits above the arc cards */}
      <div style={{
